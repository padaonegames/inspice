...
export const SelectArtworksStep = (props: StepComponentProps) => {
  ...
  const appliedFilter: GetArtworksFilter = props.getState<GetArtworksFilter>('appliedFilter', {});
  ...
  const [fetchDataStatus] = useAsyncRequest(fetchStepData, [page, itemsPerPage, appliedFilter]);

  // Whether we are in a loading state (if API requests have not been completed yet)
  const loading = !(fetchDataStatus.kind === 'success' &&
  ...
  const getPageData = (): { currentPage: number, pageTotal: number, itemsPerPage: number } | undefined => {
    if (fetchDataStatus.kind === 'success' && fetchDataStatus.result[0].kind === 'ok') {
