...
export interface ActivityCreationStageManagerProps {
...
} // ActivityCreationStageManagerProps
...
export const ActivityCreationStageManager = (
  props: ActivityCreationStageManagerProps
): JSX.Element => {
  const {
    minStages,
    currentStage,
  ...
  const editStagesEnabled =
    enableStageAddition || stages.some((elem) => elem.canBeRemoved);
      ...
      <UpperRowContainer>
        <ProgressLine
          ...
          onItemSelected={(index) => {
            if (onStageSelected) onStageSelected(index);
          }}
          ...
          removalAllowed={
            ...
            minStages <
              stages.reduce(
                (prev, curr) => prev + (curr.canBeRemoved ? 1 : 0),
                0
              )
          }
          editing={editing}
...
export default ActivityCreationStageManager;
