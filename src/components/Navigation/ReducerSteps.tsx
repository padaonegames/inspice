 ...
 */
export interface BasicStepProps<S, A, T extends StepComponentProps<S, A>> {
  /** Title of this step */
  title?: string;
...
export type StepProps<S, A, T extends StepComponentProps<S, A>> =
  BasicStepProps<S, A, T> & ExclusiveStepProps<S, A, T>;

type ExclusiveStepProps<S, A, T> = Omit<T, keyof StepComponentProps<S, A>>;
 ...
 */
export interface StepComponentProps<S, A> {
 ...
 */
function Step<S, A, T extends StepComponentProps<S, A>>(
  props: StepProps<S, A, T> & { stepContext: React.Context<StepsContext<S, A>> }
) {
  ...
  const stepsContextValue: StepsContext<S, A> = useContext(stepContext);
  ...
  if (order === currentStep) {
    const exclusiveProps: ExclusiveStepProps<S, A, T> = { ...props };
    ...
    const Component = component as ComponentType<
      StepComponentProps<S, A> & ExclusiveStepProps<S, A, T>
    >;
 ...
 */
export const generateStepsContext = <S, A>() =>
  createContext<StepsContext<S, A>>({
 ...
 */
export interface StepsProps<S, A> {
  children:
    | ReactElement<StepProps<S, A, StepComponentProps<S, A>>>
    | ReactElement<StepProps<S, A, StepComponentProps<S, A>>>[];
  config?: StepsConfig;
  state: S;
  dispatch: React.Dispatch<A>;
...
function Steps<S, A>({
  ...
  dispatch,
  stepContext,
}: StepsProps<S, A> & { stepContext: React.Context<StepsContext<S, A>> }) {
  ...
  const NavigationComponent = (context: StepsContext<S, A>) => {
  ...
  const BeforeComponent = (context: StepsContext<S, A>) => {
    if (config?.before) {
  ...
  const AfterComponent = (context: StepsContext<S, A>) => {
    if (config?.after) {
  ...
  const stepList: StepEntry[] = childSteps.map((child, order) => {
    return {
      name:
        (child as { props: StepProps<S, A, StepComponentProps<S, A>> }).props
          .title || "Step " + (order + 1),
      order: order,
    };
  });
  ...
  const context: StepsContext<S, A> = {
    stepCount,
    currentStep,
    stepList,
    state,
