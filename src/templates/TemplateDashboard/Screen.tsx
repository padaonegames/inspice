...
export const TemplateDashboard = (): JSX.Element => {
  ...
  const [
    fetchTreasureHuntActivitiesRequest,
    triggerFetchTreasureHuntActivities,
  ] = useAsyncRequest(fetchTreasureHuntActivities, []);
  ...
  if (
    fetchTreasureHuntActivitiesRequest.kind !== "success" ||
  ...
  if (
    fetchTreasureHuntActivitiesRequest.result.kind !== "ok" ||
...
const TemplateDashBoardView = (
  ...
  const activities: ActivityInstance[] = acts.map((elem) => ({
    ...
    activityType: (elem as Object).hasOwnProperty("huntDefinitionsDatasetUuid")
      ? "Treasure Hunt"
      : "GAM Game",
  }));
    ...
    switch (activity.activityType) {
      case "Treasure Hunt":
        navigate(`/find-artwork/curator/create`, {
          state: activity as FindArtworkActivityDefinition,
        });
        break;
      case "GAM Game":
        navigate(`/gam-game/curator/create`, {
          state: activity as GamGameActivityDefinition,
        });
        break;
      default:
        return;
  ...
  const handleOpenTemplate = (template: SupportedActivity) => {
    ...
    switch (template) {
      ...
      case "Multistage Form":
        navigate(`/multistage-form/curator/new-activity`);
        break;
      default:
        return;
    }
  ...
  const shouldDisplay = (activity: ActivityInstance) => {
    ...
    while (current && !current.done) {
      if (current.value[1]) {
        hasAllTags =
          activity.tags?.some((elem) => elem === current.value[0]) ?? false;
        if (!hasAllTags) break;
      }
      current = it.next();
    }
  ...
  return (
    <>
      <GridContainer>
        <GridFilters>
          <NewActivityButton
            key="new-activity-button"
            title="Create a new Activity"
            onClick={() => setNewActivityPopupOpen(true)}
          >
            <NewActivityIcon />
            <NewActivityText>New Activity</NewActivityText>
          </NewActivityButton>
            ...
            <GridFilterOption key={elem}>
              <CheckBoxInput
                ...
                onCheckedChange={(checked) =>
                  handleDisplayTypeCheck(elem, checked)
                }
