    ...
    <Root>
      ...
      <RangeInputCard
        ...
        onMinValueChange={(val) =>
          props.setState<number>("minArtworks", val, 1)
        }
        onMaxValueChange={(val) =>
          props.setState<number>("maxArtworks", val, 1)
        }
        required
        min={1}
        max={10}
      ...
      <CheckBoxGroupInputCard
        ...
        onResponseChanged={(res) =>
          props.setState<AllowedResponseType[]>(
            "allowedResponseTypes",
            (res.selectedFields as AllowedResponseType[]) ?? [],
            []
          )
        }
        response={{
          selectedFields: props.getState<AllowedResponseType[]>(
            "allowedResponseTypes",
            []
          ),
        }}
