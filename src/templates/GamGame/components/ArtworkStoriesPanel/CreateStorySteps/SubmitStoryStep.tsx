...
export const SubmitStoryStep = (props: SubmitStoryStepProps): JSX.Element => {
  ...
  const [submitStoryRequest, triggerRequest] = useAsyncRequest(
    handleSubmitStory,
    [],
    false
  );
  ...
  useEffect(() => {
    if (submitStoryRequest.kind === "success") {
      ...
      if (submitStoryRequest.result.kind === "ok") {
        ...
        setErrorMessage(
          "There was an error while submitting the story. Please try again later."
        );
      }
    }
  ...
  return (
    <StepRoot>
      <StepTitleCard stepTitle={t("submitStory")}>
        <ArtworkListDottedLine />
        <ShortTextInputCard
          fieldPayload={{}}
          width={1}
          ...
          response={{ text: title }}
          onResponseChanged={(res) => setTitle(res.text)}
        ...
        {submitStoryRequest.kind === "running" && (
          <LoadingOverlay message={t("submittingStory")} />
        )}
      </StepTitleCard>
    </StepRoot>
  );
