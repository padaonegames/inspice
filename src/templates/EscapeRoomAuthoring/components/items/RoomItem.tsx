...
export const EditableRoomItemContent = (props: EditableRoomItemContentProps): JSX.Element => {
  ...
  const [selectedBlock, setSelectedBlock] = useState<number | 'room-settings' | 'exit-block'>('room-settings');
  ...
  const currentBlock = (selectedBlock !== 'room-settings' && selectedBlock !== 'exit-block') ? blocks[selectedBlock] : undefined;
  ...
  return (
    ...
    {/* List of blocks on the top of the editor */}
      ...
      {selectedBlock === 'room-settings' && <RoomSettingsEditor hints={hints} onHintsChanged={handleHintsChanged} />}

      {/* Editor for the room's exit block */}
      {selectedBlock === 'exit-block' && <RoomExitBlockEditor exitBlock={payload.exitBlock} onPayloadChanged={handleExitBlockChanged} />  }

      {/* Editor for a block that is not the exit block */}
      {currentBlock && (selectedBlock !== 'room-settings' && selectedBlock !== 'exit-block')  && 
      <RoomBlockEditor block={currentBlock} blockIndex={selectedBlock} onPayloadChanged={handleRoomBlockChanged} />}
    </>
  );
